...
public class SearchInInfiniteSortedArray_Recursive {

	/*
	 * APPROACH : Use binary search by increasing the searching index in power of 2
	 * and if we reach the index where element could not be
	 * found(ArrayIndexOutOfBound) then search linearly
	 */
	...
	public static int searchElement(int[] arr, int x) {
		...
		} catch (ArrayIndexOutOfBoundsException arrIndexOutOfBound) {
			// search in linear way
			i = i / 2;
			return searchElementLinear(arr, x, i);
		}
		return binarySearch(arr, x, i / 2 + 1, i - 1);
	}
	
	public static int searchElementLinear(int[] arr,int x , int i) {
		try {
			while (true) {
				if(arr[i] == x) return i;
				if(arr[i] > x) return -1;
		...
		}catch(ArrayIndexOutOfBoundsException aIndOutOfBound) {
			return -1;
		}
	}
